...
######################################################################
# PyTorch Lightning DeepSpeed Stage 2 Example on Aurora
#
# BEFORE RUNNING: Edit #PBS -A line above with your project name!
#
# DeepSpeed Stage 2: ~8x memory savings (optimizer + gradients partitioned)
#
# Usage: qsub scripts/submit_deepspeed_stage2.sh
######################################################################

echo "=========================================="
echo "PyTorch Lightning DeepSpeed Stage 2 - $(wc -l < $PBS_NODEFILE) Nodes"
echo "Started: $(date)"
echo "=========================================="
...
OUTPUT_DIR="${TUTORIAL_BASE}/output/deepspeed_${MAIN_JOB_ID}"
mkdir -p ${OUTPUT_DIR}
